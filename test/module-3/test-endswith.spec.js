...
describe("Main Module Tests (src/index.spec.js)", () => {
  it("should test line transformations at the end of a string @test-endswith", async () => {
    // Before running tests: check for `String.prototype.endsWith()`.

    let foundEndsWith = false;
    ...
    expect(
      foundEndsWith,
      "In src/index.spec.js, let's use `String.prototype.endsWith()` in the second test."
    ).to.be.true;
    ...
    expect(
      ...
      "In src/index.spec.js, the second test should fail when `lf` doesn't replace line endings."
    ).to.equal(1);
    ...
    expect(
      ...
      "In src/index.spec.js, the second test should pass when `lf` replaces line endings."
    ).to.equal(0);
